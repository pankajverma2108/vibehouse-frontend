...
export function BookingPageShell({
  ...
  return (
    <section className={cn("min-h-screen bg-[#0A0A0E] pb-20 pt-24 text-white md:pb-24 md:pt-28 font-['Gilroy',sans-serif]", sectionClassName)}>
      <div className="mx-auto w-full max-w-6xl px-4 md:px-6">
        <div className="flex flex-col gap-8">
          <div className="mx-auto max-w-3xl text-center">
            {badge ? (
              <span className="inline-block border border-[var(--vh-pink)] bg-[var(--vh-pink)] px-3 py-1 text-xs font-extrabold uppercase tracking-[0.18em] text-white rounded-none shadow-[2px_2px_0px_#000000]">
                {badge}
              </span>
            ) : null}
  ...
  return (
    <div
      className={cn(
        ...
        tone === "ticket"
          ? "border-[var(--vh-pink)] bg-[#171822] text-white shadow-[6px_6px_0px_var(--vh-pink)]"
          : "border-white/15 bg-[#171822] text-white",
      )}
    >
      <div className="grid gap-6 p-6 md:grid-cols-[minmax(0,1fr)_260px] md:p-8">
        <div className="space-y-5">
          <div className="flex flex-wrap items-start justify-between gap-4 border-b border-dashed border-white/15 pb-4">
            <div>
              <p className="text-xs font-extrabold uppercase tracking-[0.18em] text-[var(--vh-pink)]">
                {eyebrow}
              </p>
              <h2 className="mt-2 font-['Cirka',serif] text-3xl uppercase tracking-tight">{title}</h2>
            </div>
            {status ? (
              <div className="border border-white/10 bg-[#12131A] px-3 py-1.5 text-xs font-extrabold uppercase tracking-[0.16em] text-[var(--vh-pink)]">
                {status.replaceAll("_", " ")}
              </div>
            ) : null}
          </div>

          <div className="grid gap-4 border border-white/10 bg-[#12131A] p-4 md:grid-cols-3">
            <div>
...
export function BookingEmptyState({
  ...
  return (
    <div className="rounded-none border border-white/15 bg-[#171822] p-8 text-center shadow-[6px_6px_0px_#000000] font-['Gilroy',sans-serif]">
