...
export function BreakfastLoadingPanel() {
  return (
    <section
      aria-busy="true"
      aria-live="polite"
      className="space-y-8 border-t border-dashed border-white/18 py-8"
      role="status"
    >
      <div className="space-y-3">
        <div className="h-4 w-32 animate-pulse rounded bg-white/10 motion-reduce:animate-none" />
        <div className="h-9 w-64 max-w-full animate-pulse rounded bg-white/10 motion-reduce:animate-none" />
        <div className="h-5 w-48 animate-pulse rounded bg-white/8 motion-reduce:animate-none" />
      </div>
      <div className="space-y-4 border-t border-dashed border-white/14 pt-7">
        {[0, 1, 2].map((item) => (
          <div
            className="h-20 animate-pulse rounded-[12px] border border-white/10 bg-white/[0.035] motion-reduce:animate-none"
            key={item}
          />
        ))}
      </div>
...
export function BreakfastErrorPanel({
  ...
  return (
    <section className="border-t border-dashed border-white/18 py-10">
      <p className="font-caption text-xs uppercase tracking-[0.12em] text-[#f3c96b]">
        Service unavailable
      </p>
      <h2 className="font-sectiontitle mt-3 text-[26px] leading-tight text-white sm:text-[32px]">
        We couldn&apos;t load your breakfast menu.
      </h2>
      <p className="mt-3 max-w-xl text-base leading-7 text-white/68">{message}</p>
      <Button
        className="mt-6 h-12 rounded-[12px] bg-[var(--vh-pink)] px-5 font-bold text-white hover:bg-[var(--vh-pink-soft)]"
        onClick={onRetry}
        type="button"
      >
...
export function BreakfastTerminalPanel({
  ...
  return (
    <section className="border-t border-dashed border-white/18 py-10">
      ...
      <h2 className="font-sectiontitle mt-3 max-w-2xl text-[28px] leading-tight text-white sm:text-[36px]">
        {copy.title}
      </h2>
