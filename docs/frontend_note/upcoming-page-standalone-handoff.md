# Upcoming Page Standalone Handoff

This note captures the current `/upcoming` page implementation as a self-contained reference for another project. The page TSX lives in `components/standalone/upcoming-page.tsx` and it depends on the shared stylesheet `components/standalone/partner-upcoming-pages.module.css`.

Use the TSX and CSS below as the structural and visual reference, then adapt typography, colors, and tokens to the target project's theme.
...
.standaloneRoot *,
.standaloneRoot *::before,
.standaloneRoot *::after {
...
.header {
  ...
  width: min(calc(100% - 112px), 1433px);
  height: 90px;
  margin: 0 auto;
  padding-top: 40px;
  padding-bottom: 24px;
...
.partnerHeroHeading span {
  display: block;
  white-space: nowrap;
}

.upcomingHeroHeading span {
  display: block;
}
...
.upcomingHeroHeading {
  max-width: 100%;
  margin: 0;
  font-size: clamp(72px, 14vw, 203px);
  line-height: 0.82;
  text-align: left;
}
...
.dispatchPanel {
  position: absolute;
  top: calc(clamp(130px, 16.5vw, 240px) + 12px);
  right: 0;
  width: 400px;
}
...
.siteCard {
  ...
  background: var(--vh-panel-soft);
}

.siteImageWrap {
  position: relative;
  aspect-ratio: 698.5 / 521.88;
  overflow: hidden;
...
.bigCta:hover .ctaArrow,
.bigCta:focus-visible .ctaArrow,
.formButton:hover .ctaArrow,
.formButton:focus-visible .ctaArrow,
.bigCta:active .ctaArrow,
.formButton:active .ctaArrow {
...
.pillarIcon {
  filter: brightness(0) saturate(100%) invert(18%) sepia(87%) saturate(3621%) hue-rotate(346deg) brightness(88%) contrast(96%);
}
...
.modelCard h2 {
  max-width: 560px;
  margin: 28px 0 42px;
  font-size: clamp(64px, 5.55vw, 85.8px);
  font-weight: 900;
  line-height: 0.85;
  color: var(--pu-ice);
}
...
@media (max-width: 767px) {
  ...
  .pillarsGrid .pillarCard:first-child {
    border-top: 0;
    padding-top: 0;
  }
