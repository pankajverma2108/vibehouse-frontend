...
## Overview
The Daily Social uses a **multi-provider authentication system**. Guests can sign up via Google OAuth, email+password, or phone OTP. Identity is centralised in the `guests` table. Authentication methods are stored as separate rows in `auth_providers`, allowing one guest to have multiple login methods.
