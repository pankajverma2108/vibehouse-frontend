...
## Overview
eZee PMS is the **source of truth for all bookings**. The Daily Social does not store bookings. When a guest enters their reservation ID, we fetch from eZee, cache it locally (`ezee_booking_cache`), and determine whether the guest is the **PRIMARY** booker or a **SECONDARY** guest using the booker's email/phone from eZee.
