...
| 6 | **FE** | Keep `Price unavailable` mitigation until live catalog/availability are no longer `ezee_only` | ⏳ In progress |
| 7 | **FE** | Verify `source: "db"` appears in catalog response after BE redeploy/cache flush | ⏳ Pending (live still `ezee_only`) |
