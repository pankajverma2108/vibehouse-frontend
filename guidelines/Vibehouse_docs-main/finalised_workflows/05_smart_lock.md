...
## Overview
Vibe House uses **MyGate digital keypad locks** on all room doors. Access is **PIN-code only** — no Bluetooth/NFC app unlock (decided as overkill). Each guest gets a unique PIN tied to their stay dates. Staff have permanent master PINs. The system tracks every unlock event via MyGate webhooks for security auditing and ghost check-in detection.
