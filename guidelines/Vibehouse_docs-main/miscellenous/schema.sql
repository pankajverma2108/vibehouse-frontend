-- ============================================================
-- Vibe House — PostgreSQL DDL Schema v4.4
-- Last synced: 2026-03-24 (matches prisma/schema.prisma)
--
-- DESIGN:
--   eZee PMS  = Source of truth for bookings, rooms, rates
--   Zoho CRM  = Source of truth for staff, tickets, SLA
...
CREATE TABLE otp_logs (
    id         VARCHAR(36) PRIMARY KEY,
    guest_id   VARCHAR(36) REFERENCES guests(id) ON UPDATE NO ACTION,
...
CREATE TABLE admin_users (
    ...
    property_id     VARCHAR(36),               -- NULL = all properties (set FK after properties table)
    is_active       BOOLEAN NOT NULL DEFAULT TRUE,
...
-- Add FK now that properties exists
ALTER TABLE admin_users
    ADD CONSTRAINT admin_users_property_id_fkey
...
CREATE TABLE room_types (
    ...
    slug                 VARCHAR(50) NOT NULL,          -- queen-private, dorm-4bed-mixed
    type                 VARCHAR(20) NOT NULL,           -- PRIVATE | DORMITORY
    total_rooms          INTEGER NOT NULL,
    beds_per_room        INTEGER NOT NULL DEFAULT 1,
    total_beds           INTEGER NOT NULL,
    base_price_per_night NUMERIC(10,2) NOT NULL,
    floor_range          VARCHAR(20),                   -- e.g. 2-3
...
CREATE TABLE events (
    ...
    description  TEXT,
    date         DATE NOT NULL,
    time         VARCHAR(10),                           -- e.g. 7:00 PM
    location     VARCHAR(200),
    capacity     INTEGER,
    price_text   VARCHAR(100),                          -- e.g. Free, ₹299
    contact_link VARCHAR(500),
    poster_url   VARCHAR(500),
    badge_label  VARCHAR(30),                           -- e.g. Tonight, New
    badge_color  VARCHAR(10),                           -- hex or color name
    is_active    BOOLEAN NOT NULL DEFAULT TRUE,
...
-- ============================================================
-- LAYER 3 — BOOKING CACHE & SLOTS (eZee is source of truth)
-- ============================================================
...
CREATE TABLE booking_slots (
    id                  VARCHAR(36) PRIMARY KEY,
    ezee_reservation_id VARCHAR(100) NOT NULL REFERENCES ezee_booking_cache(ezee_reservation_id),
    slot_number         INTEGER NOT NULL,           -- 1-indexed per reservation
    guest_id            VARCHAR(36) REFERENCES guests(id),
    label               VARCHAR(50) NOT NULL,       -- e.g. Guest 1, Guest 2
...
CREATE TABLE kyc_submissions (
    ...
    ezee_reservation_id   VARCHAR(100) NOT NULL REFERENCES ezee_booking_cache(ezee_reservation_id),
    guest_id              VARCHAR(36) NOT NULL REFERENCES guests(id),
    slot_id               VARCHAR(36) REFERENCES booking_slots(id),
    submitted_by_guest_id VARCHAR(36) REFERENCES guests(id),
...
CREATE TABLE product_catalog (
    ...
    name        VARCHAR(255) NOT NULL,
    description TEXT,
    category    VARCHAR(20) NOT NULL,           -- COMMODITY | SERVICE | BORROWABLE | RETURNABLE
...
CREATE TABLE borrowable_checkouts (
    ...
    issued_by_admin_id                 VARCHAR(255) REFERENCES admin_users(id)
);

CREATE TABLE returnable_checkouts (
    id                            VARCHAR(36) PRIMARY KEY,
    inventory_id                  VARCHAR(36) NOT NULL REFERENCES inventory(id),
    addon_order_item_id           VARCHAR(36) NOT NULL REFERENCES addon_order_items(id),
    ezee_reservation_id           VARCHAR(100) NOT NULL REFERENCES ezee_booking_cache(ezee_reservation_id),
    guest_id                      VARCHAR(36) NOT NULL REFERENCES guests(id),
    unit_code                     VARCHAR(20),
    quantity                      INTEGER NOT NULL DEFAULT 1,
    ...
    returned_at                   TIMESTAMP(6),
    returned_verified_by_admin_id VARCHAR(36) REFERENCES admin_users(id),
    condition_on_return           VARCHAR(15),   -- GOOD | DAMAGED | LOST
    status                        VARCHAR(15) NOT NULL DEFAULT 'ISSUED',  -- ISSUED | RETURNED | OVERDUE | LOST
    issued_by_admin_id            VARCHAR(36) REFERENCES admin_users(id),
    notes                         TEXT
);
...
CREATE TABLE admin_activity_log (
    ...
    actor_id    VARCHAR(100) NOT NULL,  -- admin_users.id or 'SYSTEM'
    action      VARCHAR(50) NOT NULL,
