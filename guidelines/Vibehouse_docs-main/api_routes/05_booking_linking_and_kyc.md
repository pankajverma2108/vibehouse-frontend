...
## MODULE 2b: eZee Reconciliation & Auto-Link (Background)

These are **not API endpoints** — they are background processes that run automatically.
Understanding them is essential for testing the booking lifecycle.

---

### Reconciliation Service

**File:** `src/ezee/ezee-reconciliation.service.ts`

Runs on application bootstrap and every **15 minutes** thereafter. Three passes:

| Pass | What it does |
|---|---|
| **1. Unsynced bookings** | Finds `VH-*` bookings with `status=CONFIRMED` but no `ezee_reservation_no` (payment captured but eZee sync failed) → re-queues `INSERT_BOOKING` to SQS |
| **2. State drift** | For every active booking with an `ezee_reservation_no`, fetches the booking from eZee and compares status/room. Updates our cache if eZee shows Cancelled, Checked In, Checked Out, No Show, or a different room number |
| **3. External ingestion** | Fetches all eZee reservations arriving yesterday → 30 days out. Creates `ezee_booking_cache` entries for any reservations we don't already have (OTA bookings, walk-ins, staff-created). ERIs are generated as `EZEE-{CITY_CODE}-{reservationNo}` (e.g. `EZEE-MUM-56`) |

**ERI format for external bookings:**
- VibeHouse-originated: `VH-BANDRA-MN2Y5HHX-B647`
- eZee-originated (OTA/walk-in): `EZEE-MUM-56`

Both formats are treated identically by all downstream systems (KYC, check-in, payments, smart locks).

**Fields populated from eZee for external bookings:**
- `booker_email`, `booker_phone` — used for auto-link matching
- `room_type_name`, `room_number`, `checkin_date`, `checkout_date`
- `source` — e.g. "Walk-in", "MakeMyTrip", "Booking.com", "Internet Booking Engine"
- `no_of_guests`, `status`

---

### Auto-Link on Login / Signup

**File:** `src/guest/auth/guest-auth.service.ts`

When a guest signs up, logs in (email/password), or authenticates via Google OAuth,
the system automatically checks if any `ezee_booking_cache` entries have a matching
`booker_email` or `booker_phone`. If found and not already linked, a
`booking_guest_access` row is created with:
- `role = PRIMARY` if email or phone matches `booker_email`/`booker_phone`
- `role = SECONDARY` otherwise
- `status = APPROVED`

This means:
1. Guest books via MakeMyTrip with `vatsal@gmail.com`
2. Reconciliation ingests the booking from eZee → `EZEE-MUM-56`, `booker_email=vatsal@gmail.com`
3. Guest downloads the app, signs up with `vatsal@gmail.com`
4. Auto-link fires → `booking_guest_access` created as PRIMARY
5. `GET /guest/booking/mine` immediately returns the MMT booking

**Auto-link is fire-and-forget** — it never blocks the auth response. Failures are
logged but the guest still gets their token. They can always link manually via
`POST /guest/booking/link`.

---

### Manual Link (unchanged)

If auto-link didn't fire (e.g. guest used a different email than the one on the OTA booking),
the guest can still call:
...
This works for both `VH-*` and `EZEE-*` ERIs.
